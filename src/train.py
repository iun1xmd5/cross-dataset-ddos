...
Everything that is not model training lives in the sibling modules:
config.py, data_loader.py, feature_selection.py, preprocessing.py, sequence_builder.py,
class_weighting.py and evaluate.py.
...
from class_weighting import RF_CLASS_WEIGHT, keras_class_weights, xgboost_sample_weights
from config import (
    ALL_MODELS,
    NEURAL_MODELS,
    Config,
...
def build_lstm(input_shape: Tuple[int, int]):
    """Three stacked LSTM layers (256-128-64) + dense head."""
    tf, layers, _, _, regularizers = _keras()
...
def build_bilstm(input_shape: Tuple[int, int]):
    """Three stacked bidirectional LSTM layers + dense head."""
    tf, layers, _, _, regularizers = _keras()
    reg = regularizers.l2(1e-4)
    inp = tf.keras.Input(shape=input_shape, name="sequence_input")
...
def build_cnn_lstm(input_shape: Tuple[int, int]):
    """Conv1D feature extractor followed by one LSTM layer."""
    tf, layers, _, _, _ = _keras()
    inp = tf.keras.Input(shape=input_shape, name="sequence_input")
