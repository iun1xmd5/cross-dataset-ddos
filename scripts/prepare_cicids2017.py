...
# -*- coding: utf-8 -*-
"""
Prepare CICIDS2017 for the cross-dataset transfer experiment.
...
What it does (identical row selection to the notebook / cross_dataset_transfer.py):
  ...
  3. Replaces +/-inf with NaN (CICIDS2017 has many in Flow Bytes/s and
     Flow Packets/s) and drops rows with missing values.
  4. Upper-cases labels and keeps only BENIGN and DDoS. All other attack families
     (PortScan, DoS Hulk, Bot, web attacks, ...) are discarded, because the
     experiment is DDoS vs. benign.
  5. Keeps the first ``--per-class-cap`` rows of each class.
  6. Writes ``<output>/cicids2017_processed.csv`` and ``<output>/summary.json``.
...
Examples
--------
    python scripts/prepare_cicids2017.py                        # uses data/raw/manifest.json
    python scripts/prepare_cicids2017.py --input /data/cicids2017 --per-class-cap 15000
"""
...
# Only these labels (after upper-casing) survive; "DDoS" is the sole attack class.
KEEP_LABELS = {"BENIGN", "DDOS"}
...
log = logging.getLogger("prepare_cicids2017")
...
def build(input_dir: Path, per_class_cap: int, chunk_size: int) -> Dict:
    ...
    for path in csv_paths:
        ...
        for chunk in pd.read_csv(
            ...
            low_memory=False,
            on_bad_lines="skip",
            encoding_errors="replace",  # some CICIDS2017 files contain non-UTF-8 bytes in labels
        ):
            ...
            mask = chunk[OUT_LABEL].isin(KEEP_LABELS)
            if (~mask).any():
...
def main() -> int:
    ...
    parser.add_argument("--input", type=Path, default=None,
                        help="raw CICIDS2017 directory (default: from data/raw/manifest.json)")
    ...
    parser.add_argument("--output", type=Path, default=REPO_ROOT / "data" / "processed" / DATASET_KEY,
                        help="output directory (default: <repo>/data/processed/cicids2017)")
    ...
    log.info("  BENIGN=%d  ATTACK(DDoS)=%d", n_benign, len(df) - n_benign)
    for label, cnt in summary["kept_per_class"].items():
        log.info("    %-20s %7d", label, cnt)
    if res["discarded"]:
        log.info("  Discarded labels: %s", res["discarded"])
